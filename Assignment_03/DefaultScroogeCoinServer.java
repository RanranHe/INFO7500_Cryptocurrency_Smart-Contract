...
public class DefaultScroogeCoinServer implements ScroogeCoinServer {
	...
	// For every 10 minute epoch, this method is called with an unordered list of
	// proposed transactions
	// submitted during this epoch.
	// This method goes through the list, checking each transaction for correctness,
	// and accepts as
	...
	public synchronized Set<UTXO> getUTXOs() {
		...
		for (int ledgerindex = 0; ledgerindex < ledger.size(); ledgerindex++) {
			...
			case Create:
				...
				break;
			case Pay:
				for (int i = 0; i < trans.numInputs(); i++) { // input
					Transaction.Input in = trans.getInputs().get(i);
                    int indexOut = in.getIndexOfTxOutput();
				...
				for (Transaction.Output op : trans.getOutputs()) {// output
